...
export async function stage(role, input, output, work, image, { signal } = {}) {
  signal?.throwIfAborted();
  if (!["analyze", "generate"].includes(role))
    throw new Error("Unknown isolated role");
  if (!/^sha256:[a-f0-9]{64}$/.test(image))
    throw new Error("Docker image must be pinned to local immutable ID");
  const tool = path.join(work, `${role}-tool`);
  await mkdir(tool, { recursive: true });
  await mkdir(output, { recursive: true });
  await cp(
    path.join(root, "workers", `${role}.mjs`),
    path.join(tool, `${role}.mjs`),
  );
  if (role === "analyze") {
    await cp(
      path.join(root, "workers/vision.mjs"),
      path.join(tool, "vision.mjs"),
    );
    await mkdir(path.join(tool, "node_modules"), { recursive: true });
    await cp(
      path.join(root, "node_modules/pngjs"),
      path.join(tool, "node_modules/pngjs"),
      { recursive: true },
    );
  }
...
fs.writeFileSync('/output/boundary-probe.json',JSON.stringify({denied,readOnlyInput:true,networkConnectionDenied:true,history:'Fresh deterministic process; no conversation or credentials supplied'}));`;
  ...
  const inputManifest = await manifest(input),
    toolManifest = await manifest(tool);
  ...
  const args = [
    ...
    "--mount",
    `type=bind,src=${path.resolve(input)},dst=/input,readonly`,
    "--mount",
    `type=bind,src=${tool},dst=/tool,readonly`,
    "--mount",
    `type=bind,src=${path.resolve(output)},dst=/output`,
    image,
  ];
  const audit = {
    role,
    ...
    policy: {
      ...
      readOnlyRoot: true,
      capabilities: "none",
      mounts: ["input:ro", "tool:ro", "output:rw"],
      history: "none",
    },
    cleanup: false,
  };
  try {
    await exec("docker", [...args, "node", "/tool/probe.mjs"], {
      timeout: 15000,
      maxBuffer: 1024 * 1024,
      signal,
    });
    const result = await exec(
      "docker",
      [...args, "node", `/tool/${role}.mjs`, "/input", "/output"],
      { timeout: 300000, maxBuffer: 1024 * 1024, signal },
    );
    audit.stdout = result.stdout;
    audit.stderr = result.stderr;
    if (JSON.stringify(await manifest(input)) !== JSON.stringify(inputManifest))
      throw new Error("Stage inputs changed");
    audit.outputManifest = await manifest(output);
    audit.success = true;
  } catch (error) {
    audit.success = false;
    audit.failure = String(error.message).slice(0, 4000);
    throw error;
  } finally {
    try {
      ...
      if (String(error.stderr).includes("No such container"))
        audit.cleanup = true;
      else audit.cleanupError = String(error.message);
    }
