...
const spec=JSON.parse(await readFile(path.join(process.argv[2],'specification.json')));
const output=process.argv[3];
for(const k of ['movementRate','mouseRadiansPerPixel','jumpImpulse','gravity'])if(!Number.isFinite(spec.parameters[k])||spec.parameters[k]<=0)throw new Error(`Invalid inferred ${k}`);
...
await writeFile(path.join(output,'requirements.json'),JSON.stringify(spec.requirements,null,2));
await writeFile(path.join(output,'index.html'),'<!doctype html><html lang="en"><meta charset="utf-8"><title>Gameparse reconstruction</title><style>body{margin:0;background:#182b30;overflow:hidden;color:white;font:14px system-ui}aside{position:fixed;top:20px;left:24px;pointer-events:none}</style><canvas width="1280" height="720"></canvas><aside>Reconstruction · Drag to aim · WASD move · Space jump · R reset</aside><script type="module" src="game.mjs"></script></html>');
await writeFile(path.join(output,'implementation-map.json'),JSON.stringify({claims:spec.claims.map(c=>({specificationId:c.id,implementation:'game.mjs'})),generatorAdditions:['background/grid/crosshair styling','pitch clamp ±1 rad','repeat jumping while held','flat ground, no collision geometry'],modelFamily:'Constant-rate movement, relative mouse rotation, ballistic jump; not arbitrary game synthesis'},null,2));
