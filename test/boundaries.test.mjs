...
test("manifest rejects symlink escapes instead of hashing private host content", async () => {
  ...
  try {
    ...
    await assert.rejects(manifest(directory), /Symlink/);
  } finally {
...
test("server serves only explicitly allowed game routes", async () => {
  ...
  let server;
  try {
    await writeFile(path.join(directory, "index.html"), "game");
    await writeFile(path.join(directory, "private.json"), "private");
...
test("final cases use new event sequences on the declared input clock grid", () => {
  ...
  for (const random of [() => 0, () => 0.3, () => 0.6, () => 0.99])
    for (const s of finalCases(random)) {
      assert.ok(!existing.has(JSON.stringify(s.events)));
      assert.ok(s.events.every((e) => e.at % 16 === 0 && e.at <= s.duration));
    }
});
